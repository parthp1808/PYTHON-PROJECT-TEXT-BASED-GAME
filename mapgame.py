...
class mapgame:
    ...
    def __init__(self):
        loader=fileloader()
        self.characterf=loader.fileloaderinst('character')
        self.creatures=loader.fileloaderinst('creature')
        self.items=loader.fileloaderinst('item')
        self.map1dic=loader.fileloaderinst('map')
        self.nuggetdic=loader.fileloaderinst('nugget')
    def charactercreator(self):
        
        objlist=[]
        for keyz in self.characterf.keys():
            keyz1=character(keyz,self.characterf[keyz]['STATS'].get('magicalres'),self.characterf[keyz]['STATS'].get('armor'),self.characterf[keyz].get('location'),self.characterf[keyz]['ABILITIES'],self.characterf[keyz].get('State'))
    ...
    def creaturecreator(self):#create objects for the creatures
        ...
        for keyz in self.creatures.keys():
            if self.creatures[keyz].get('location')!="":
                keyz1=creature(keyz,self.creatures[keyz]['STATS'].get('magicalres'),self.creatures[keyz]['STATS'].get('armor'),self.creatures[keyz].get('location'),self.creatures[keyz].get('ABILITIES'),self.creatures[keyz]['STATS'].get('STATE'))
                objlist.append(keyz1)
        return objlist
    def itemcreator(self,name):
       
        statsitems=self.items[name]['STATS']
        obj=item(name,statsitems.get('heal'),statsitems.get('physicaldmg'),statsitems.get('magicaldmg'),statsitems.get('magicalres'),statsitems.get('armor'),statsitems.get('location'),statsitems.get('FIGHT'),statsitems.get('verb'),statsitems.get('State'))
    ...
    def nuggetcreator(self):##WORJK ON THIS STUFF
        ...
        for keyz in self.nuggetdic.keys():
            keyz1=nuggets(keyz,self.nuggetdic[keyz]['LOOK'],self.nuggetdic[keyz]['verb'],self.itemcreator(self.nuggetdic[keyz]['items']),self.nuggetdic[keyz]['LOCATION'])
            objlist.append(keyz1)
        return objlist
