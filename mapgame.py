...
class mapgame:
    ...
    def charactercreator(self):
        ...
        for keyz in self.characterf.keys():
            keyz1=character(keyz,'1',self.itemcreator(self.characterf[keyz]['items']),self.characterf[keyz]['STATS'].get('magicalres'),self.characterf[keyz]['STATS'].get('armor'),self.characterf[keyz].get('location'),self.characterf[keyz]['ABILITIES'],self.characterf[keyz].get('State'))
    ...
    def creaturecreator(self):#create objects for the creatures
        ...
        for keyz in self.creatures.keys():
            if self.creatures[keyz].get('location')!="":
                keyz1=creature(keyz,self.creatures[keyz]['LOOK'],self.creatures[keyz]['STATS'].get('magicalres'),self.creatures[keyz]['STATS'].get('armor'),self.creatures[keyz].get('location'),self.creatures[keyz].get('ABILITIES'),self.creatures[keyz]['STATS'].get('STATE'))
                objlist.append(keyz1)
        return objlist
