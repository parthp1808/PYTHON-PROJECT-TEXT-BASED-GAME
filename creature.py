...
class creature(actor):  # creature class
    def __init__(self, name, magicalres, armor, location, abilities, state):
        ...
        self.state = state
        self.info = loader.creaturedicinstance()
    ...
    def givedamage(self, opponent, magicresreduction, armorreduction, physicaldmg,
                   magicaldmg):  # possibly override.give damage to opponent
    ...
    def usespell(self, opponent):  # use spell and do damage to the player
        if self.state == True and self.location == opponent.location:
            ...
            for num, val in enumerate(arr):
                if num == rand:
                    self.givedamage(opponent, arr[val].get('magicresreduction'), arr[val].get('armorreduction'),
                                    arr[val].get('physicaldmg'), arr[val].get('magic damage'))
